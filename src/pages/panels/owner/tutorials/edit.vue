...
function submit() {
  ...
  tutorialsApi
    ...
    .then(() => {
      notifySuccess("Tutorial atualizado.");
      router.push({ name: "owner.tutorials" });
    })
    .catch(() => notifyError("Não foi possível atualizar o tutorial."))
...
onMounted(() => {
  ...
  Promise.all([tutorialCategoriesApi.plucks(), tutorialsApi.getById(nid)])
    .then(([plucks, res]) => {
      categorySelectOptions.value = [
        { value: 0, text: "Selecione uma categoria…", disabled: true },
        ...plucks.map((c) => ({ value: c.id, text: c.name })),
      ];
      const t = res.tutorial;
      if (!t) throw new Error("empty");
      tutorial_category_id.value = t.tutorial_category_id;
      title.value = t.title;
      url.value = t.url;
      thumbnail.value = t.thumbnail;
      targets.value = (t.targets ?? []).map((x) => x.target as TutorialTargetCode);
    })
    .catch(() => {
      notifyError("Tutorial não encontrado.");
      cancel();
    })
  ...
  <DefaultLayout>
    <div class="py-4">
      <div class="d-flex flex-wrap align-items-start justify-content-between gap-2 mb-4">
        <div>
          <h1 class="h4 fw-semibold mb-1">Editar tutorial</h1>
          <p class="text-muted small mb-0">Atualize o conteúdo e a imagem de capa no catálogo.</p>
        </div>
        <b-button variant="outline-secondary" size="sm" :disabled="loading || loadState" @click="cancel">
          Cancelar
        </b-button>
      </div>

      <div v-if="loadState" class="text-body-secondary py-5 text-center">
        Carregando…
      </div>
