...
export type LoadUsersForCompanyOptions = {
  /** Incluir este usuário na lista mesmo já tendo funcionário (ex.: edição do vínculo atual) */
  exceptEmployeeUserId?: number;
};

/**
 * Usuárioes da empresa ainda sem funcionário (para vínculo opcional em owner/company).
 */
...
export type LoadUsersForEmployeeParams = {
  companyId: number;
  /** Se definido, só usuárioes com acesso a esta filial (`branch_users`) */
  branchId?: number;
  /** Só quem ainda não tem funcionário; incluir exceção (ex.: usuário já ligado a este funcionário na edição) */
  exceptEmployeeUserId?: number;
};

/**
 * Usuárioes elegíveis para vincular a um funcionário: mesma empresa,
