...
import type { ScaleTypeFormData } from "@/core/schemas";

const props = withDefaults(
  defineProps<{
    modelValue: ScaleTypeFormData;
...
const emit = defineEmits<{
  (e: "update:modelValue", value: ScaleTypeFormData): void;
...
const localForm = computed({
  ...
  set: (value: ScaleTypeFormData) => emit("update:modelValue", value),
});
...
function updateField<K extends keyof ScaleTypeFormData>(field: K, value: ScaleTypeFormData[K]) {
  localForm.value = {
    ...localForm.value,
    [field]: value,
  };
...
<template>
  <UIComponentCard title="Dados do tipo de escala">
    <b-row class="g-3">
      ...
      <b-col md="12">
        <b-form-group label="Nome" label-for="scale-type-name">
          <b-form-input
            id="scale-type-name"
            :model-value="modelValue.name"
            type="text"
            placeholder="Nome do tipo de escala"
            :readonly="isView"
