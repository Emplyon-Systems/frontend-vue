...
async function submit() {
  const valid = validateDayOffModalityForm(form.value, "edit");
  if (!valid.success) {
    errors.value = valid.errors as Record<string, string>;
    return;
  }
  await dayOffModalitiesApi.update(id.value, valid.data);
...
<template>
  <component :is="isOwnerWorkspace ? 'div' : DefaultLayout">
    <div class="py-4">
      <h1 class="h4 mb-3">Editar modalidade de folga</h1>
