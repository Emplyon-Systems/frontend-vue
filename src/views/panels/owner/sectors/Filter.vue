...
export interface SectorsFilterModel {
  name: string;
  company_ids: number[];
  branch_ids: number[];
...
const emit = defineEmits<{
  (e: "update:modelValue", value: SectorsFilterModel): void;
  (e: "apply"): void;
  (e: "reset"): void;
}>();
...
function update<K extends keyof SectorsFilterModel>(field: K, value: SectorsFilterModel[K]) {
...
async function applyFilters() {
  const next: Partial<SectorsFilterModel> = { ...props.modelValue };
  ...
  emit("update:modelValue", next as SectorsFilterModel);
  await nextTick();
  emit("apply");
}
  ...
  <div class="p-0">
    <b-row class="g-3">
      <b-col v-if="showCompanyFilter && companyOptions?.length" md="4">
        <b-form-group label="Empresa" label-for="filter-sector-company">
          <select
            id="filter-sector-company"
      ...
      <b-col md="4">
        <b-form-group label="Nome do setor" label-for="filter-sector-name">
          <b-form-input
            id="filter-sector-name"
      ...
      <b-col v-if="!hideBranchSelector && (branchOptions?.length ?? 0) > 0" md="4">
        <b-form-group label="Filial" label-for="filter-sector-branch">
          <select
            id="filter-sector-branch"
            ref="branchSelectRef"
            class="form-select"
            multiple
          >
      ...
      <b-col md="6">
        <b-form-group label="Data de cadastro de" label-for="filter-sector-created-from">
          <b-form-input
            id="filter-sector-created-from"
      ...
      <b-col md="6">
        <b-form-group label="Data de cadastro até" label-for="filter-sector-created-until">
          <b-form-input
            id="filter-sector-created-until"
