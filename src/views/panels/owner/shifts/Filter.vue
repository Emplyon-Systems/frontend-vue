...
export interface ShiftsFilterModel {
  name: string;
  company_ids: number[];
  branch_ids: number[];
...
const props = withDefaults(
  ...
  { showCompanyFilter: true, hideBranchSelector: false }
);

const emit = defineEmits<{
  (e: "update:modelValue", value: ShiftsFilterModel): void;
  (e: "apply"): void;
  (e: "reset"): void;
}>();
...
function update<K extends keyof ShiftsFilterModel>(field: K, value: ShiftsFilterModel[K]) {
...
async function applyFilters() {
  const next: Partial<ShiftsFilterModel> = { ...props.modelValue };
  ...
  emit("update:modelValue", next as ShiftsFilterModel);
  await nextTick();
  emit("apply");
}
...
<template>
  <div class="p-0">
    <b-row class="g-3">
      <b-col v-if="showCompanyFilter && (companyOptions?.length ?? 0) > 0" md="4">
        <b-form-group label="Empresa" label-for="filter-shift-company">
          <select id="filter-shift-company" ref="companySelectRef" class="form-select" multiple>
            <option
          ...
          <div class="d-flex justify-content-between align-items-center mt-1">
            ...
            <b-button v-if="modelValue.company_ids.length" type="button" variant="link" size="sm" class="p-0" @click="clearCompanySelection">
              Limpar seleção
            </b-button>
          </div>
        </b-form-group>
      ...
      <b-col md="4">
        <b-form-group label="Nome do turno" label-for="filter-shift-name">
          <b-form-input
            id="filter-shift-name"
      ...
      <b-col v-if="!hideBranchSelector && (branchOptions?.length ?? 0) > 0" md="4">
        <b-form-group label="Filial" label-for="filter-shift-branch">
          <select id="filter-shift-branch" ref="branchSelectRef" class="form-select" multiple>
            <option
            ...
            >
              {{ branch.name }}{{ branch.company_name ? ` (${branch.company_name})` : "" }}
            </option>
          </select>
          <div class="d-flex justify-content-between align-items-center mt-1">
            <small class="text-muted">
              {{ modelValue.company_ids?.length ? "Filiais das empresas selecionadas" : selectionLabel(modelValue.branch_ids.length, "filial", "filiais") }}
            </small>
            <b-button v-if="modelValue.branch_ids.length" type="button" variant="link" size="sm" class="p-0" @click="clearBranchSelection">
              Limpar seleção
            </b-button>
          </div>
        </b-form-group>
      ...
      <b-col md="6">
        <b-form-group label="Data de cadastro de" label-for="filter-shift-created-from">
          <b-form-input
            id="filter-shift-created-from"
      ...
      <b-col md="6">
        <b-form-group label="Data de cadastro até" label-for="filter-shift-created-until">
          <b-form-input
            id="filter-shift-created-until"
