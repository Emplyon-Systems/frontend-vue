...
export interface ModalityTypesFilterModel {
  name: string;
  company_ids: number[];
  branch_ids: number[];
...
const props = defineProps<{
  modelValue: ModalityTypesFilterModel;
  active?: boolean;
...
const emit = defineEmits<{
  (e: "update:modelValue", value: ModalityTypesFilterModel): void;
  (e: "apply"): void;
  (e: "reset"): void;
}>();
...
function update<K extends keyof ModalityTypesFilterModel>(field: K, value: ModalityTypesFilterModel[K]) {
...
async function applyFilters() {
  const next: Partial<ModalityTypesFilterModel> = { ...props.modelValue };
  ...
  emit("update:modelValue", next as ModalityTypesFilterModel);
  await nextTick();
  emit("apply");
}
  ...
  <div class="p-0">
    <b-row class="g-3">
      <b-col v-if="showCompanyFilter && companyOptions?.length" md="4">
        <b-form-group label="Empresa" label-for="filter-modality-type-company">
          <select
            id="filter-modality-type-company"
      ...
      <b-col md="4">
        <b-form-group label="Nome" label-for="filter-modality-type-name">
          <b-form-input
            id="filter-modality-type-name"
      ...
      <b-col v-if="!hideBranchSelector && (branchOptions?.length ?? 0) > 0" md="4">
        <b-form-group label="Filial" label-for="filter-modality-type-branch">
          <select
            id="filter-modality-type-branch"
            ref="branchSelectRef"
            class="form-select"
            multiple
          >
      ...
      <b-col md="6">
        <b-form-group label="Data de cadastro de" label-for="filter-modality-type-created-from">
          <b-form-input
            id="filter-modality-type-created-from"
      ...
      <b-col md="6">
        <b-form-group label="Data de cadastro até" label-for="filter-modality-type-created-until">
          <b-form-input
            id="filter-modality-type-created-until"
