...
import type { PositionFormData } from "@/core/schemas";

const props = withDefaults(
  defineProps<{
    modelValue: PositionFormData;
...
const emit = defineEmits<{
  (e: "update:modelValue", value: PositionFormData): void;
...
const localForm = computed({
  ...
  set: (value: PositionFormData) => emit("update:modelValue", value),
});

function updateField<K extends keyof PositionFormData>(field: K, value: PositionFormData[K]) {
...
<template>
  <UIComponentCard title="Dados do cargo">
    <b-row class="g-3">
      <b-col v-if="!isBranchLocked" md="12">
        <b-form-group label="Filial" label-for="position-branch_id">
          <select
            id="position-branch_id"
            ref="branchSelectRef"
            class="form-select"
      ...
      <b-col md="12">
        <b-form-group label="Nome" label-for="position-name">
          <b-form-input
            id="position-name"
