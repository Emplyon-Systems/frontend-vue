...
function fillFormFromEmployee(data: Awaited<ReturnType<typeof employeesApi.getById>>) {
  ...
  const assignments =
    (e.branches ?? []).length > 0
      ? (e.branches ?? []).map((b) => ({
          ...
          modality_type_id: Number(b.pivot?.modality_type_id ?? 0),
          is_primary: !!b.pivot?.is_primary,
        }))
      : [{ branch_id: 0, sector_id: 0, modality_type_id: 0, is_primary: true }];
  form.value = {
  ...
  if (c?.id != null && (c.name ?? "").trim()) {
    companyOptions.value = [
      { id: c.id, name: c.name!.trim(), internal_email_domain: c.internal_email_domain },
    ];
