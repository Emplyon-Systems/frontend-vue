...
export interface EmployeesFilterModel {
  search: string;
  company_ids: number[];
  branch_ids: number[];
...
const props = defineProps<{
  modelValue: EmployeesFilterModel;
  active?: boolean;
...
const emit = defineEmits<{
  (e: "update:modelValue", value: EmployeesFilterModel): void;
  (e: "apply"): void;
  (e: "reset"): void;
}>();
...
function update<K extends keyof EmployeesFilterModel>(field: K, value: EmployeesFilterModel[K]) {
...
async function applyFilters() {
  const next: Partial<EmployeesFilterModel> = { ...props.modelValue };
  ...
  emit("update:modelValue", next as EmployeesFilterModel);
  await nextTick();
  emit("apply");
}
  ...
  <div class="p-0">
    <b-row class="g-3">
      <b-col v-if="showCompanyFilter && companyOptions?.length" md="4">
        <b-form-group label="Empresa" label-for="filter-emp-company">
          <select id="filter-emp-company" ref="companySelectRef" class="form-select" multiple>
            <option
      ...
      <b-col md="4">
        <b-form-group label="Pesquisa" label-for="filter-emp-search">
          <b-form-input
            id="filter-emp-search"
            :model-value="modelValue.search"
            type="text"
            placeholder="Nome, e-mail, CPF, telefone ou cargo"
            @update:model-value="update('search', String($event ?? ''))"
          />
        </b-form-group>
      </b-col>
      <b-col v-if="!hideBranchSelector && (branchOptions?.length ?? 0) > 0" md="4">
        <b-form-group label="Filial" label-for="filter-emp-branch">
          <select id="filter-emp-branch" ref="branchSelectRef" class="form-select" multiple>
            <option
          ...
          <div class="d-flex justify-content-between align-items-center mt-1">
            <small class="text-muted">
              {{
                showCompanyFilter && modelValue.company_ids?.length
                  ? "Filiais das empresas selecionadas"
      ...
      <b-col md="6">
        <b-form-group label="Data de cadastro de" label-for="filter-emp-created-from">
          <b-form-input
            id="filter-emp-created-from"
      ...
      <b-col md="6">
        <b-form-group label="Data de cadastro até" label-for="filter-emp-created-until">
          <b-form-input
            id="filter-emp-created-until"
